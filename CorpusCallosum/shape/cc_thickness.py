...
def convert_to_ras(contour, vox2ras_matrix, get_parameters=False):
    # converting to AS (no left-right dimension), out of plane movement is ignores, so we only do scaling, axes swapping and flipping - no rotation
...
def find_closest_edge(point, contour):
    ...
    t[valid_edges] = np.sum((point - edges_start[valid_edges]) * edges_vec[valid_edges], axis=1) / edge_lengths_sq[valid_edges]
...
def insert_point_to_contour(contour_with_thickness, point, thickness_value, get_index=False):
    ...
    contour_with_thickness[0] = np.insert(contour_with_thickness[0], edge_idx+1, point, axis=0)
...
def cc_thickness(contour_2d, anterior_endpoint_idx, posterior_endpoint_idx, n_points=100):

    # standardize contour indices, to get consistent levelpath directions
    contour_2d, anterior_endpoint_idx, posterior_endpoint_idx = set_contour_zero_idx(contour_2d, anterior_endpoint_idx, anterior_endpoint_idx, posterior_endpoint_idx)
    ...
    level = 0
    midline_equidistant, midline_length = tria.level_path(vfunc, level, n_points=n_points+2)
    ...
    gf_interp = scipy.interpolate.griddata(tria.v[:,0:2], gf, midline_equidistant[:,0:2], method='cubic')
    ...
    for i in range(1,n_points+1):
        ...
        lvlpath, lvlpath_length, tria_idx = tria.level_path(gf, level, get_tria_idx=True)
    ...
    # ax.scatter(cont[:,0][anterior_endpoint_idx], cont[:,1][anterior_endpoint_idx], c='r', label='Anterior Endpoint', marker='o')
    # ax.scatter(cont[:,0][posterior_endpoint_idx], cont[:,1][posterior_endpoint_idx], c='b', label='Posterior Endpoint', marker='o')
    # ax.legend()
    # plt.show()
    ...
    # thickness_measurement_points = np.concatenate([thickness_measurement_points_top, thickness_measurement_points_bottom], axis=0).T
    ...
    # ax2.plot(thickness_measurement_points[0, :100], -thickness_measurement_points[1, :100], 'ro', markersize=3, label='Thickness Points (start)')
    # ax2.plot(thickness_measurement_points[0, 100:], -thickness_measurement_points[1, 100:], 'go', markersize=3, label='Thickness Points (end)')
    ...
    #         ax.plot(levelpaths[i][:,0], levelpaths[i][:,1], 'r-', marker='o', markersize=0) # , label=f'Level {levelpath_lengths[i]:.2f}'
    # ax.plot(midline_equidistant[:,0], midline_equidistant[:,1], '-', label='Midline', alpha=1, color='darkgoldenrod')#, marker='o', markersize=2)
